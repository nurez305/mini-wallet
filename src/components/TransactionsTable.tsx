  ...
  return (
    <div className="w-full">
      ...
      <button
        ...
        className="md:hidden mb-4 w-full bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 py-2 px-4 rounded-lg flex items-center justify-center space-x-2"
      >
        <Filter size={16} />
        <span>Filters</span>
          ...
          <div className="flex flex-col space-y-3">
            <div className="flex items-center space-x-2">
              <input 
                ...
                className="flex-1 border rounded px-3 py-2 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:placeholder-gray-400" 
              />
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-3">
              <select 
                ...
                className="border rounded px-3 py-2 text-sm flex-1 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
              >
                {categories.map((c) => (
                  <option key={c} value={c} className="dark:bg-gray-700 dark:text-white">
                    {c}
                  </option>
                ))}
              </select>
              <button
                ...
                className="px-4 py-2 text-sm bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded hover:bg-gray-50 dark:hover:bg-gray-600 dark:text-white"
              >
                Clear Filters
              </button>
            </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 dark:bg-gray-700 sticky top-0">
                <tr>
                  ...
                  <th className="text-left px-4 py-3 dark:text-gray-300">Account</th>
                  <th className="text-left px-4 py-3 dark:text-gray-300">Merchant</th>
                  <th className="text-left px-4 py-3 dark:text-gray-300">Category</th>
                  <th className="text-right px-4 py-3 dark:text-gray-300">Amount</th>
                  <th className="text-right px-4 py-3 dark:text-gray-300">Balance</th>
                </tr>
              </thead>
                ...
                {filtered.map((t) => {
                  ...
                  return (
                    <tr key={t.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                      <td className="px-4 py-3 dark:text-gray-300">{dayjs(t.date).format("DD/MM/YYYY")}</td>
                      <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">
                        {account?.name || `Account ${t.accountId}`}
                      </td>
                      <td className="px-4 py-3 dark:text-gray-300">{t.merchant}</td>
                      <td className="px-4 py-3 dark:text-gray-300">{t.category}</td>
